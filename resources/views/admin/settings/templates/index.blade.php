            ...
            <div class="col-md-9">
                <table class="b-table w-100 sortable-table mt-6">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <span>{{ $row->name }} {{ __('Template') }}</span>
                                <br>
                                <small class="text-muted">{{ __('Subject') }}: {{ $row->subject }}</small>
                            </td>
                            <td>{{ date('d-M-y', strtotime($row->modified_at)) }}</td>
                            <td>
                                <a href="{{ url('admin/settings/email-templates/edit/'.$row->id) }}" class="btn btn-sm btn-primary js-view-template">{{ __('Edit') }}</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
